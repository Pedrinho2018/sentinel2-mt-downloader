...
def localizar_tif(pasta: Path, nome: str) -> Path | None:
    candidatos = list(pasta.glob(f"{nome}.*"))
...
def gerar_preview(b04: np.ndarray, b03: np.ndarray, b02: np.ndarray, mascara: np.ndarray,
                   destino: Path, pmin: float, pmax: float, qualidade: int) -> None:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Gera patches georreferenciados para catalogação de soja.")
    ...
    tamanho = int(pcfg.get("tamanho_px", 256))
    passo = int(pcfg.get("passo_px", tamanho))
    nuvem_max = float(pcfg.get("nuvem_max_pct", 10))
    valid_min = float(pcfg.get("dados_validos_min_pct", 85))
    ...
    qualidade_jpeg = int(cfg.get("preview", {}).get("qualidade_jpeg", 92))
        ...
        try:
            ...
            for y in range(0, ref.height - tamanho + 1, passo):
                for x in range(0, ref.width - tamanho + 1, passo):
                    ...
                    mascara = ~np.isin(scl, list(CLASSES_INVALIDAS | CLASSES_NUVEM_SOMBRA))
                    gerar_preview(b04, b03, b02, mascara, preview, pmin, pmax, qualidade_jpeg)
                    ...
                    aprovados += 1
                    print(f"[OK] {patch_id} | nuvem+sombra={nuvem_pct:.1f}% | válidos={valid_pct:.1f}%")
    ...
    campos = [
        "patch_id", "scene_id", "data", "row", "col", "xoff", "yoff", "width", "height",
        "cloud_shadow_pct", "valid_data_pct", "minlon", "minlat", "maxlon", "maxlat",
        "centroid_lon", "centroid_lat", "preview", "label", "observacao"
    ]
